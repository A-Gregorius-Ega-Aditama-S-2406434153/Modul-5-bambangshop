...
This tutorial uses the push model of the Observer pattern. In the current implementation, the publisher creates a `Notification` payload containing `product_title`, `product_type`, `product_url`, `subscriber_name`, and `status`, then sends that payload directly to each subscriber by calling `Subscriber::update` with an HTTP `POST` request. That means the publisher is actively pushing the event data to observers, instead of merely signaling that "something changed" and waiting for subscribers to fetch the details themselves. Because the notification already contains the important data, subscribers do not need to query the publisher again just to understand what happened.

If we used the pull model instead, one advantage would be a lighter notification message, because the publisher could send only a minimal event signal or product identifier and let each subscriber fetch fresh data afterward. That can reduce duplicated payload data and gives subscribers more flexibility if they want to retrieve only certain fields. However, for this tutorial, the pull model would also introduce extra HTTP requests and tighter runtime dependence on the publisher's availability. After receiving a notification, each subscriber would still need to call the publisher again to get product details, which increases latency and creates more points of failure. In this project, the push model is more practical because the publisher already knows the event details at notification time and can send everything in one step.

If we decide not to use multithreading in the notification process, the program would still work functionally, but notification delivery would become synchronous and slower. Right now, `NotificationService::notify` spawns a separate thread for each subscriber, so sending to one slow subscriber does not block sending to the others. Without multithreading, the publisher would notify subscribers one by one and wait for each HTTP request to finish before continuing. As a result, product creation, deletion, or publish actions could become noticeably slower, especially when there are many subscribers or when some subscriber endpoints respond slowly. In the worst case, one slow or unreachable subscriber could delay the whole request path, so multithreading improves responsiveness and makes the notification fan-out more scalable for this design.
